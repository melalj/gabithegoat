...
class Store {
  ...
  async loadSkus(product_id) {
    try {
      const response = await fetch(`/api/products/${product_id}/skus`);
  ...
  async updatePaymentIntentWithShippingCost(
    ...
    try {
      const response = await fetch(
        `/api/payment_intents/${paymentIntent}/shipping_change`,
        {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({
  ...
  async displayPaymentSummary() {
    ...
    for (let [id, product] of Object.entries(this.products)) {
      const qs = document.location.href.split('qty=');
      const quantity = qs[1] ? Number(qs[1].split('&')[0]) : 1;
